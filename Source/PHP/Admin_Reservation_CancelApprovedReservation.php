...
if (isset($data['reservation_id'])) {
    ...
    $sql = "UPDATE Transactions SET Transaction_status = 'CANCELED', Transaction_current = 0 
            WHERE Transaction_Reserve_id = ?";

    // Prepare and execute the statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters
        $stmt->bind_param("i", $reservation_id); // Assuming Transaction_Reserve_id is an integer
